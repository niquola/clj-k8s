     ...
     (if (and (map? x) (contains? x :valueFrom))
       (if-let [{nm :name k :key} (get-in x [:valueFrom :secretKeyRef])]
         (if-let [res (secret nm (keyword k))]
           res
           (do (println "Could not resolve secret " k " from " nm) x))
         x)
       x)) res))
...
(defn patch [nres]
  (let [res (find nres)]
    (if-not (or (nil? res) (number? res) (= "Failure" (get res :status)))
...
(defn ws-listener
  ...
  (reify WebSocketListener
    ...
    (onWebSocketBinary [this bytes offset len]
      (println "Session" (ByteBuffer/wrap bytes offset len)))
    (onWebSocketError [this cause]
      (when-not (println "not error ch!!!"))
      (when cause
        (println "ERROR:" cause)))
...
(comment
  ...
  @(http-client/get
    (str kube-url "/api/v1/pods")
    {:headers default-headers :insecure? true})

  (def ws
    (WebSocketClient.
     "wss://<api-server>/api/v1/namespaces/default/pods/pg3-cleo-master-lightseagreen-363763885-0mbfz/exec?command=ls"
     ))


  @(http-client/post
    (str kube-url "/api/v1/namespaces/default/pods/pg3-cleo-master-lightseagreen-363763885-0mbfz/exec?command=ls")
    {:headers default-headers :insecure? true})

  (def ws (WebSocketClient.  (SslContextFactory/trustAnybody)))

  (def req (ClientUpgradeRequest.))
  ...
  (def conn
    (.connect ws (ws-listener)
              (java.net.URI. "wss://<api-server>/api/v1/namespaces/default/pods/pg3-cleo-master-lightseagreen-363763885-0mbfz/exec?command=ls")
              req))
